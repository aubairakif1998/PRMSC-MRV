...
const WaterDrafts = () => {
  ...
  const [drafts, setDrafts] = useState<WaterDraft[]>([]);
  ...
  const loadData = async () => {
    try {
      ...
      setDrafts((draftsRes.drafts as WaterDraft[]) || []);
      setSystems((systemsRes as WaterSystem[]) || []);
    } catch (error: unknown) {
      setToast({
        message: getApiErrorMessage(error, "Sync failed. Please check connection."),
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  const handleSubmitDraft = async (draftId: number) => {
    ...
    try {
      await submitWaterDraft(draftId);
      setToast({ message: "Submission published successfully!", type: "success" });
      const draftsRes = await getWaterDrafts();
      setDrafts((draftsRes.drafts as WaterDraft[]) || []);
    } catch (error: unknown) {
      setToast({
        message: getApiErrorMessage(error, "Batch submission failed"),
        type: "error",
      });
    } finally {
  ...
  const handleDeleteSystem = async (systemId: number) => {
    ...
    try {
      await deleteWaterSystem(systemId);
      setToast({ message: "Asset decommissioned", type: "success" });
      await loadData();
    } catch (error: unknown) {
      setToast({
        message: getApiErrorMessage(error, "Failed to decommission asset"),
        type: "error",
      });
    } finally {
  ...
  const handleDeleteDraft = async (draftId: number) => {
    ...
    setDeleting((prev) => ({ ...prev, [key]: true }));
    try {
      await deleteWaterDraft(draftId);
      setToast({ message: "Draft purged from local state", type: "success" });
      await loadData();
    } catch (error: unknown) {
      setToast({
        message: getApiErrorMessage(error, "Purge failed"),
        type: "error",
      });
    } finally {
      setDeleting((prev) => ({ ...prev, [key]: false }));
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">
                Manage infrastructure and pending monthly submissions
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-[2fr_1fr]">
          <Card className="border-slate-200">
            <CardHeader className="flex flex-row items-center justify-between">
              <div className="space-y-1">
                ...
                <CardDescription>
                  Complete and publish your pending monthly water entries.
                </CardDescription>
              </div>
              ...
              {drafts.length === 0 ? (
                <div className="rounded-xl border border-dashed border-slate-200 p-10 text-center">
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    All monthly reports are either submitted or not started.
                  </p>
                </div>
              ) : (
                        ...
                        <CardContent className="flex flex-col gap-4 p-4 lg:flex-row lg:items-center lg:justify-between">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            <Button
                              ...
                              className="gap-1.5"
                              onClick={() =>
                                navigate(`/operator/water-data?draft=${draft.id}`)
                              }
                            >
                            ...
                            <Button
                              ...
                              onClick={() =>
                                openDeleteDialog({
                                  ...
                                  description:
                                    "This will permanently remove the selected water draft.",
          ...
          <Card className="border-slate-200">
            <CardHeader>
              ...
              <CardDescription>
                Registered systems available for draft data entry.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {systems.length === 0 ? (
                        ...
                        <CardContent className="flex items-center justify-between gap-3 p-3">
                          <div>
                            <p className="text-[11px] font-bold uppercase tracking-wide text-sky-700">
                              UID: {system.unique_identifier?.substring(0, 8) ?? "N/A"}
                            </p>
                            <p className="text-sm font-semibold text-slate-900">
                              {system.village || "Undefined"}
                            </p>
                          ...
                          <div className="flex gap-2">
                            <Button
                              ...
                              size="icon-sm"
                              onClick={() =>
                                navigate(`/operator/water-data?system=${system.id}`)
                              }
                            >
                            ...
                            <Button
                              ...
                              onClick={() =>
                                openDeleteDialog({
                                  ...
                                  description:
                                    "This will remove the selected water system and related historical logs.",
                                  actionLabel: "Delete System",
                                })
                              }
                            >
