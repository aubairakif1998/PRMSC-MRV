...
import { MySubmissionsLoading } from "./screenSkeletons";
...
function getSubmittedAt(item: Submission): unknown {
  return (
    item.submitted_at ?? item.submission_date ?? item.created_at ?? null
  );
}
...
function submissionKind(type: unknown): "water" | "solar" | "other" {
...
export function MySubmissionsScreen() {
  ...
  const renderItem: ListRenderItem<Submission> = useCallback(
    ...
    const TypeIcon = kind === "water" ? Droplets : kind === "solar" ? Sun : Inbox;
    const iconColor =
      kind === "water" ? "#0369a1" : kind === "solar" ? "#c2410c" : "#64748b";
    ...
    >
      <View className="flex-1 px-4 pt-4">
        ...
        <Tabs
          value={status}
          onValueChange={(v) =>
            setStatus(v as (typeof STATUS)[number])
          }
        >
                      ...
                      {queuedItems.slice(0, 4).map((q) => (
                        ...
                        >
                          <Text className="text-sm font-medium text-amber-950 dark:text-amber-100">
                            {q.type === "water" ? "Water log" : "Solar log"}
                          </Text>
                ...
                <Card className="border-dashed bg-muted/40 py-10">
                  <CardContent className="items-center gap-2">
                    ...
                    <CardDescription className="text-center">
                      Nothing matches this filter yet, or you have not submitted
                      from this account.
                    </CardDescription>
                  </CardContent>
                </Card>
